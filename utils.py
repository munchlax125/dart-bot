...
def format_analysis_result(text: str) -> str:
    """분석 결과를 HTML 형식으로 변환 (개선된 버전)"""
    # HTML 이스케이프 방지 - Gemini가 이미 마크다운으로 응답하므로 그대로 사용
...
def create_summary_stats(financial_data: Dict) -> Dict[str, Any]:
    ...
    # 3. 소제목들(**텍스트**: 형태) 간의 가독성을 위한 줄바꿈 추가
    # 볼드 처리 후 적용해야 함
    text = re.sub(
...
def create_summary_stats(financial_data: Dict) -> Dict[str, Any]:
    ...
    return summary,
        r'<div style="margin: 8px 0; padding: 8px 12px; background: #ffffff; border: 1px solid #e9ecef; border-radius: 4px;"><span style="color: #667eea; font-weight: bold;">•</span> \1</div>',
        text,
        flags=re.MULTILINE
    )
...
def create_summary_stats(financial_data: Dict) -> Dict[str, Any]:
    ...
    return summary,
        '<hr style="margin: 25px 0; height: 2px; background: #667eea; border: none; border-radius: 2px;">', 
        text, 
        flags=re.MULTILINE
    )
    text = re.sub(
...
def create_summary_stats(financial_data: Dict) -> Dict[str, Any]:
    ...
    # 7. 이모지 처리
    text = re.sub(r'(⭐+)', r'<span style="color: #ffd700; font-size: 16px;">\1</span>', text)
    text = re.sub(r'(✅|❌|⚠️|🔍|📊|📈|📉|💰|🏢|🔧|💡|🎯|📋)', r'<span style="font-size: 16px; margin-right: 3px;">\1</span>', text)
    
    # 8. 자연스러운 줄바꿈 처리 - 단락 구분만
    # 두 개 이상의 연속된 줄바꿈만 <br><br>로 변환
    text = re.sub(r'\n{2,}', '<br><br>', text)
    
    # 단일 줄바꿈은 공백으로 처리 (자연스러운 문장 연결)
    text = re.sub(r'\n(?![<])', ' ', text)
    
    return f'<div style="line-height: 1.8; font-size: 15px; color: #333; padding: 20px; background: #ffffff; border-radius: 8px;">{text}</div>'
